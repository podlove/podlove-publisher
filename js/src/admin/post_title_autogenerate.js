jQuery(document).ready(function ($) {
  if (window.PODLOVE.override_post_title && window.PODLOVE.override_post_title.enabled) {
  ...
  function podlove_update_episode_title() {
    var template = window.PODLOVE.override_post_title.template

    var mnemonic = window.PODLOVE.override_post_title.mnemonic
    ...
    if (episode_title) {
      ...
      title = title.replace(
        '%episode_number%',
        padLeft(episode_number, window.PODLOVE.override_post_title.episode_padding, '0')
      )
      title = title.replace('%season_number%', window.PODLOVE.override_post_title.season_number)
      title = title.replace('%episode_title%', episode_title)
      $titleinput.val(title)
    } else {
      $titleinput.attr('placeholder', window.PODLOVE.override_post_title.placeholder)
    }
