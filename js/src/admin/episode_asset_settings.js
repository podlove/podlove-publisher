 ...
 */
;(function ($) {
  window.PODLOVE.EpisodeAssetSettings = function (container) {
    // private
    var o = {}
    ...
    function filter_file_formats_by_asset_type() {
      $('select[name=podlove_episode_asset_type]', container)
        .on('change', function () {
          var $container = $(this).closest('table')

          $('#option_storage option').remove().appendTo($('#podlove_episode_asset_file_type_id'))
          $("#podlove_episode_asset_file_type_id option[data-type!='" + $(this).val() + "']")
            .remove()
            .appendTo($('#option_storage'))
    ...
    function generate_default_episode_asset_title() {
      $('select[name*=file_type_id]', container).on('change', function () {
        ...
        var isCreateAction =
          $container.closest('form').find("input[name='action']").val() === 'create'

        if (!fileFormatTitle) return
    ...
    o.update_preview = function () {
      $('#url_preview', container).each(function () {
        ...
        var format_extension = $container
          .find('[name*="file_type_id"] option:selected')
          .data('extension')
        ...
        template = template.replace(
          '%media_file_base_url%',
          '<span style="color:grey">' + media_file_base_uri
        )
