...
;(function ($) {
  window.PODLOVE.License = function (settings) {
    ...
    var podlove_change_url_preview_and_name_from_form = function (
      version_value,
      modification_value,
      commercial_use_value,
      jurisdiction_value
    ) {
      ...
      $('.podlove_podcast_license_image').html(
        podlove_get_license_image(version_value, modification_value, commercial_use_value)
      )
      $('.row_podlove_podcast_license_preview').show()
    }

    var podlove_get_license_image = function (
      version_value,
      modification_value,
      commercial_use_value
    ) {
      if (version_value == 'cc0') {
        ...
        return (
          ...
          settings.plugin_url +
          '/images/cc/' +
          podlove_license_cc_get_image(modification_value, commercial_use_value) +
          '.png" alt="" />'
        )
      }
    }
    ...
    var podlove_filter_license_selector = function (license_version) {
      switch (license_version) {
        case 'cc3':
          $(
            '#license_cc_allow_modifications, #license_cc_allow_commercial_use, #license_cc_license_jurisdiction'
          )
            .closest('div')
            .show()
          break
        case 'cc4':
          $('#license_cc_allow_modifications, #license_cc_allow_commercial_use')
            .closest('div')
            .show()
    ...
    var podlove_populate_license_form = function (
      version_value,
      modification_value,
      commercial_use_value,
      jurisdiction_value
      ...
      $('#license_cc_version')
        .find('option[value=' + version_value + ']')
        .attr('selected', 'selected')
      $('#license_cc_allow_modifications')
        .find('option[value=' + modification_value + ']')
        .attr('selected', 'selected')
      $('#license_cc_allow_commercial_use')
        .find('option[value=' + commercial_use_value + ']')
        .attr('selected', 'selected')
      $('#license_cc_license_jurisdiction')
        .find('option[value=' + jurisdiction_value + ']')
        .attr('selected', 'selected')
      ...
      if (
        $(settings.license_name_field_id).val() == '' ||
        $(settings.license_url_field_id).val() == ''
      )
    ...
    $(
      '#license_cc_allow_modifications, #license_cc_allow_commercial_use, #license_cc_license_jurisdiction, #license_cc_version'
    ).on('change', function () {
    ...
    $(document).ready(function () {
      if (
        ...
        podlove_populate_license_form(
          ...
          settings.license.modification,
          settings.license.commercial_use,
          settings.license.jurisdiction
        )
