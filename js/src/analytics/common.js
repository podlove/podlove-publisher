...
PODLOVE.Analytics.formatThousands = function (num) {
  ...
  else return PODLOVE.Analytics.decimalRound(num / 1000, 1) + 'k'
}
...
PODLOVE.Analytics.hourFormat = function (hours) {
  ...
  if (weeks) label.push(PODLOVE.Analytics.decimalRound(weeks, 1) + 'w')

  if (days) label.push(PODLOVE.Analytics.decimalRound(days, 1) + 'd')

  if (hours) label.push(PODLOVE.Analytics.decimalRound(hours, 1) + 'h')

  if (label.length === 0) label = ['0h']
