...
jQuery(document).ready(function ($) {
  ...
  function render_episode_performance_chart() {
    ...
    var episodeGroup = episodeDimension.group().reduce(
      function (p, v) {
        p.downloads += v.downloads
        p.episode_id = v.episode_id
        return p
    ...
    var downloadsWithoutTopGroup = dateDimension.group().reduce(
      function (p, v) {
    ...
    for (var index in top_episode_ids) {
      top_episode_groups[top_episode_ids[index]] = filter_dimension_by_episode_id(
        dateDimension,
        top_episode_ids[index]
      )
    }
    ...
    var downloadsChart = dc
      ...
      .title(function (d) {
        return [titleDateFormat(d.key), 'Downloads: ' + d.value.downloads].join('\n')
      })
  ...
  function load_episode_performance_chart() {
    if (totalsRawData) {
      ...
      $.when($.ajax(ajaxurl + '?action=podlove-analytics-total-downloads-per-day')).done(function (
        csvTotals
      ) {
  ...
  function render_abo_total() {
    ...
    var q1Dimension = episodeDimension
      .group()
      .reduce(aboReduceAdd('q1'), aboReduceRemove('q1'), aboReduceInit)
    var d1Dimension = episodeDimension
      .group()
      .reduce(aboReduceAdd('d1'), aboReduceRemove('d1'), aboReduceInit)
    var w1Dimension = episodeDimension
      .group()
      .reduce(aboReduceAdd('w1'), aboReduceRemove('w1'), aboReduceInit)
    ...
    chart
      ...
      .title(function (d) {
        return [aboTotalsRawData[d.key].title, 'Downloads: ' + d.value].join('\n')
      })
      .renderHorizontalGridLines(true)
  ...
  function load_abo_total() {
    if (aboTotalsRawData) {
      ...
      $.when($.ajax(ajaxurl + '?action=podlove-analytics-csv-episodes-table')).done(function (
        csvTotals
      ) {
        var i = 0
        var csvMapper = function (d) {
  ...
  const renderAssetsChart = function (data) {
    ...
    const total = xf
      .groupAll()
      .reduceSum((d) => d.downloads)
      .value()

    let chart = dc
      .rowChart('#analytics-chart-global-assets')
      .margins({
        top: 0,
        left: 40,
        right: 10,
  ...
  const renderSourcesChart = function (data) {
    ...
    let chart = dc
      .rowChart('#analytics-chart-global-sources')
      .margins({
        top: 0,
        left: 40,
        right: 10,
  ...
  const renderClientsChart = function (data) {
    ...
    let chart = dc
      .rowChart('#analytics-chart-global-clients')
      .margins({
        top: 0,
        left: 40,
        right: 10,
  ...
  const renderSystemsChart = function (data) {
    ...
    let chart = dc
      .rowChart('#analytics-chart-global-systems')
      .margins({
        top: 0,
        left: 40,
        right: 10,
  ...
  const renderTopEpisodesChart = function (data) {
    ...
    let chart = dc
      .rowChart('#analytics-global-top-episodes')
      .margins({
        top: 0,
        left: 40,
        right: 10,
  ...
  const renderPerMonthChart = function (data) {
    ...
    let chart = dc
      .lineChart('#analytics-chart-global-downloads-per-month')
      .margins({
        top: 0,
        left: 40,
        right: 10,
  ...
  const loadDownloadsCount = (from, to) => {
    ...
    $.when(
      $.ajax(
        ajaxurl +
          '?action=podlove-analytics-global-total-downloads' +
          '&date_from=' +
          from.toDateString() +
          '&date_to=' +
          to.toDateString()
  ...
  const loadShowsTable = (from, to) => {
    ...
    if (showsWrapper.length) {
      ...
      $.when(
        $.ajax(
          ajaxurl +
            '?action=podlove-analytics-global-total-downloads-by-show' +
            '&date_from=' +
            from.toDateString() +
            '&date_to=' +
            to.toDateString()
