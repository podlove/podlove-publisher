...
export const reducer = handleActions({
  [INIT]: (state: State, action: typeof init): State => ({
    ...state,
    metadata: {
      enable_episode_explicit: get(action, ['payload', 'expert_settings', 'metadata', 'enable_episode_explicit'], null) === 1,
      enable_episode_license: get(action, ['payload', 'expert_settings', 'metadata', 'enable_episode_license'], null) === 1,
      enable_episode_recording_date: get(action, ['payload', 'expert_settings', 'metadata', 'enable_episode_license'], null) === 1,
    },
    tracking: {
    ...
    website: {
      blog_title_template: get(action, ['payload', 'expert_settings', 'website', 'blog_title_template'], null),
      custom_episode_slug: get(action, ['payload', 'expert_settings', 'website', 'custom_episode_slug'], null),
      enable_generated_blog_post_title: get(action, ['payload', 'expert_settings', 'website', 'enable_generated_blog_post_title'], null) === 'on',
      episode_archive: get(action, ['payload', 'expert_settings', 'website', 'episode_archive'], null) === 'on',
      episode_archive_slug: get(action, ['payload', 'expert_settings', 'website', 'episode_archive_slug'], null),
      episode_number_padding: get(action, ['payload', 'expert_settings', 'website', 'episode_number_padding'], null),
      feeds_skip_redirect: get(action, ['payload', 'expert_settings', 'website', 'feeds_skip_redirect'], null) === 'on',
      hide_wp_feed_discovery: get(action, ['payload', 'expert_settings', 'website', 'hide_wp_feed_discovery'], null) === 'on',
      landing_page: get(action, ['payload', 'expert_settings', 'website', 'landing_page'], null),
      merge_episodes: get(action, ['payload', 'expert_settings', 'website', 'merge_episodes'], null) === 'on',
      ssl_verify_peer: get(action, ['payload', 'expert_settings', 'website', 'ssl_verify_peer'], null) === 'on',
      url_template: get(action, ['payload', 'expert_settings', 'website', 'url_template'], null),
      use_post_permastruct:get(action, ['payload', 'expert_settings', 'website', 'use_post_permastruct'], null) === 'on',
    }
  })
}, initialState);
