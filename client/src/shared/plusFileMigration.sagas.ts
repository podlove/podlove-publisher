...
type CreateApi = () => any
type OnFileMigrated = ((filename: string) => any) | null

export const createPlusFileMigrationRootSaga = (
  createApi: CreateApi,
  onFileMigrated: OnFileMigrated = null
) => {
  function* plusFileMigrationSaga(): Generator<any, void, any> {
  ...
  function* migrateFile(
    ...
    try {
      ...
      if (response.result === false) {
        ...
        if (onFileMigrated) {
          yield put(onFileMigrated(currentFileName))
        }
      }
    } catch (error) {
      yield put(plusFileMigration.setFileState({ filename: currentFileName, state: 'error' }))
  ...
  function* startMigration(api: any): Generator<any, void, any> {
    ...
    const allMigrationTasks = episodesWithFiles.flatMap((episode, episodeIndex) =>
      episode.files.map((_file, fileIndex) => ({ episodeIndex, fileIndex }))
    )
