...
class MultiLine
{
   ...
   static textWidth(c, t)
   {
      var x = 0;
      for (var i = 0; i < t.length; i++)
      {
         var w = c.measureText(t[i]).width;
         if (x < w)
            x = w;
      }
      return x;
   ...
   mouse_move_handler(e)
   {
      ...
      for (i = 0; i < this.rdata_.length; i++)
      {
         ...
         if (mouseX >= this.rdata_[i].x1 && mouseY <= this.rdata_[i].y1 && mouseX <= this.rdata_[i].x2 && mouseY >= this.rdata_[i].y2)
         {
            ...
            this.tx_canvas_.width = MultiLine.textWidth(this.tctx_, tx) + 10;
            this.tx_canvas_.height = tx.length * 15 + 10;
            this.tctx_.clearRect(0, 0, this.tx_canvas_.width, this.tx_canvas_.height);
            MultiLine.fillText(this.tctx_, tx, 5, 15, 15);
            break;
         }
      }
