...
(function($){
	...
	PODLOVE.AuphonicImport = function () {
		 ...
		 function create_auphonic_production() {
		 	...
		 	xhr.onload = function(e) {
		 		var response = JSON.parse(e.target.response),
		 			...
		 			files = document.querySelector('.auphonic_local_upload_url');

		 		$.publish('/auphonic/createproduction/success', [production]);

		 		$(".auphonic_services").each( function(index, track) {
		 			service = $("#auphonic_services_" + $(track).data("trackid")).val();
		 			if (service  == "file" && files) {
		 				$.publish('/auphonic/fileupload', [production, $button, $(track).data("trackid")] );
		 			} else {
		 				$.publish('/auphonic/fileexternalupload', [production, service, $(track).data("trackid")] );
		 			}
		 		} );
		 	...
		 	$.publish('/auphonic/createproduction/before');
	 	 	$.ajax({
	 	 		...
	 	 		success: function(publisher_podcast) {
	 	 			...
	 	 			$(".auphonic_services").each( function(index, track) {
	 	 				// Add all files that will be uploaded to the production
	 	 				if ($(track).val() == 'file') {
	 	 					trackid = $(track).data("trackid");
	 	 					var track = {
	 	 						type: 'multitrack',
	 	 						id: $("#auphonic_track_" + trackid).val()
	 	 					};
	 	 			...
	 	 			auphonic_production_data.multi_input_files = tracks;
		...
		function fetch_files_for_current_service(servicesid) {
			var trackid = $(servicesid).data("trackid"),
				uuid = $(servicesid).val(),
				api_url = "https://auphonic.com/api/service/{uuid}/ls.json".replace("{uuid}",uuid),
				$files = $("#auphonic_production_files_" + trackid),
				$button = $("#fetch_auphonic_production_files_" + trackid).button_with_loading_indicator();

			if (uuid == "url") {
				$("#auphonic_http_upload_url_" + trackid).show();
				$("#auphonic_production_files_" + trackid + ", #auphonic_local_upload_url_" + trackid).hide();
				$("#fetch_auphonic_production_files_" + trackid).hide();
			} else if (uuid == "file") {
				$("#auphonic_local_upload_url_" + trackid).show();
				$("#auphonic_production_files_" + trackid + ", #auphonic_http_upload_url_" + trackid).hide();
				$("#fetch_auphonic_production_files_" + trackid).hide();
			} else {
				$("#auphonic_http_upload_url_" + trackid + ", #auphonic_local_upload_url_" + trackid).hide();
				$("#auphonic_production_files_" + trackid ).show();
				$("#fetch_auphonic_production_files_" + trackid).show();
				$button.trigger('start');
				$.getJSON(api_url, { bearer_token: PODLOVE.Auphonic.get_token() }, function(data) {
					if (data && data.status_code == 200) {
						$files.empty();
		...
		function fetch_incoming_services(trackid) {
			var api_url = "https://auphonic.com/api/services.json",
				$services = $("#auphonic_services_" + trackid);
			...
			$.getJSON(api_url, { bearer_token: PODLOVE.Auphonic.get_token() }, function(data) {
				if (data.status_code == 200) {
					...
					fetch_files_for_current_service("#auphonic_services_" + trackid);
				}
			}).fail(function() {
		...
		if ($("#auphonic").length) {
			...
			$("#auphonic_productions").change(function(e) {
				...
				if ($production.val() !== "0") {
					...
					// update "chosen"
					$("#auphonic_productions").trigger("liszt:updated");
			...
			$("#add_new_auphonic_track").on('click', function() {
				template = $("#podlove-auphonic-track-row").html();
				template = template.replace(/\{\{id\}\}/g, $counter);

				$("#podlove-auphonic-tracks-table-body").append(template);

				fetch_incoming_services($counter);

				// Register change event for new Auphonic Services Dropdown
				$("#auphonic_services_" + $counter).change(function(){
					fetch_files_for_current_service("#" + $(this).attr("id"));
				});
				// Register event for the files "reload button"
				$("#fetch_auphonic_production_files_" + $counter).click(function(){
					fetch_files_for_current_service("#auphonic_services_" + $(this).data("trackid"));
				});
	...
	/** 
	 * Auphonic Service File Uploader
	 *
	 * This module handles the file opload of media files from external services to Auphonic.
	 */
	PODLOVE.AuphonicFileExternalServiceUploader = (function() {
		    	
		var init = function() {
			$(document).ready(function () {
		   		$.subscribe('/auphonic/fileexternalupload', doUpload);
		    });
		};

		var doUpload = function(event, production, service, trackid) {
			var xhr = PODLOVE.Auphonic.createCORSRequest("POST", "https://auphonic.com/api/production/{uuid}/multi_input_files.json".replace('{uuid}', production.uuid));
			xhr.setRequestHeader("Content-type","application/json");
			xhr.setRequestHeader("Authorization","Bearer " + PODLOVE.Auphonic.get_token());
			...
			$.ajax({
	 	 		...
	 	 		success: function(publisher_podcast) {
	 	 			serviceData = {
	 	 				input_file: $("#auphonic_production_files_" + trackid).val(),
	 	 				type: 'multitrack',
	 	 				id: $("#auphonic_track_" + trackid).val()
	 	 			};

	 	 			if ( service == 'url' )
	 	 				serviceData.input_file = $("#auphonic_http_upload_url_" + trackid).val();
    ...
    PODLOVE.AuphonicFileUploader = (function() {
    	...
    	var getFile = function(trackid) {
    		return document.querySelector('#auphonic_local_upload_url_' + trackid).files[0];
    	};

    	var doUpload = function(event, production, button, trackid) {
			...
			formData.append($("#auphonic_track_" + trackid).val(), getFile(trackid));
