...
(function($){
	...
	PODLOVE.AuphonicImport = function () {
		...
		function extract_auphonic_data_from_form(mode) {
			...
			data.webhook = site_url + "/?podlove-auphonic-production=" + $("#post_ID").val();
		...
		if ($("#auphonic").length) {
			...
			// Create Auphonic publish event
			$("#auphonic_publish_after_finishing").on( 'change', function( e ) {
				if ( !$(this).is(":checked") )
					return;
				
				var data = {
					action: "podlove-add-production-remote-publish",
					post_id: $("#post_ID").val()
				}
