...
(function($){
	...
	PODLOVE.AuphonicImport = function () {
		...
		function fetch_auphonic_production_status(production_uuid) {
			...
			$.getJSON(api_url, { bearer_token: get_token() }, function(data) {
				if (data && data.status_code == 200) {
					update_production_status(data.data);
				}
			});
		 ...
		 */				
		 function create_auphonic_production() {
		 	...
		 	var $status = $("#auphonic-production-creation-status");
		 	...
		 	xhr.onload = function(e) {
		 		...
		 		$status.html("Production created");

		 		if (service == "file" && file) {

		 			$.publish('/auphonic/fileupload', [ production_uuid, get_token(), $status, $button ] );

		 			// $status.html("File Upload");
		 			...
		 			// 	$status.html("File Upload " + Math.round(percent) + "%");
		 			// }, false);
		 			// xhr2.onload = function(e) {
		 			// 	$button.trigger('stop');
		 			// 	$status.html("File Uploaded");
		 			// 	$status.delay(750).fadeOut(200);
		 			// };
		 	...
		 	$status.html("Creating production").show();
		}
		...
		function fetch_episodes() {
			...
			var productions = $.getJSON(url, function(data) {
				if (data) {
					...
					$("#import_from_auphonic").append('<option value="0">Select existing Production</option>\n');
					$(auphonic_productions).each(function(key, value) {				
						...
						$("#import_from_auphonic").append('<option value="' + value.uuid + '">' + option_title + '</option>\n');
						$option = $("#import_from_auphonic option:last");
						$option.data("title", production_title);
						$option.data("date", date);
						$option.data("datestring", datestring);
						$option.data("status", value.status_string);
						$option.data("status_number", value.status);
					});
					jQuery("#import_from_auphonic").change();
		...
		if ($("#auphonic").length) {
			...
			$("#import_from_auphonic").change(function(e) {
				...
				if ($production.val() !== "0") {
					...
					$("#auphonic-selected-production, #auphonic-creation-status-wrapper, #auphonic-the-production .production").show();
				} else {
					$("#auphonic-selected-production, #auphonic-creation-status-wrapper, #auphonic-the-production .production").hide();
				}
	...
	var auphonicFileUploader;

	$(document).ready(function () {
		auphonicFileUploader = new AuphonicFileUploader();
		new AuphonicCreateProductionStatus();
    });

    var AuphonicFileUploader = function() {
    	var $service;
    	this.init();
    };

    AuphonicFileUploader.prototype = function() {
    	var init = function() {
    	...
    	var doUpload = function(event, production_uuid, token, status, button) {
    ...
    var AuphonicCreateProductionStatus = function() {
    	var $status;
    	this.init();
    };

    AuphonicCreateProductionStatus.prototype = function() {

    	var init = function() {
    		$status = $("#auphonic-production-creation-status");
    		$.subscribe('/auphonic/fileupload/before', updateBefore)
    		$.subscribe('/auphonic/fileupload/progress', updateProgress)
    		$.subscribe('/auphonic/fileupload/done', updateDone);
    	};

    	var updateBefore = function(e, production) {
    		$status.html("File Upload");;
    	};

    	var updateProgress = function(e, production, percent) {
    		$status.html("File <3 Upload " + Math.round(percent) + "%");
    	};

    	var updateDone = function(e, production) {
    		$status.html("File Uploaded");
    		$status.delay(750).fadeOut(200);
    	};
