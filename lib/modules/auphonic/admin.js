	...
	PODLOVE.AuphonicImport = function () {
		...
		function fetch_episodes() {
			var $button = $("#reload_productions_button").button_with_loading_indicator(),
				url = 'https://auphonic.com/api/productions.json?bearer_token={token}'.replace('{token}', PODLOVE.Auphonic.get_token());
    ...
    PODLOVE.AuphonicCreateProductionStatus = (function() {
    	...
    	var init = function() {
			$(document).ready(function () {
	    		...
	    		$.subscribe('/auphonic/createproduction/start', updateProductionStart)
	    		// create production
