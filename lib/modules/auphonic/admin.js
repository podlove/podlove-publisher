...
(function($){
	...
	PODLOVE.AuphonicImport = function () {
				...
				function(data) {
					if (data) {
						...
						if (data.data) {
							...
							update_production_status(production_data);
		...
		function update_production_status(production_data) {
			var status_text = production_data.status_string,
				$status = $("#auphonic-creation-status");

			if (production_data.status_string === "Error") {
				status_text += " (" + production_data.error_message + ")";
			}

			$status.html(status_text);
			switch (production_data.status_string) {
				case "Incomplete Form":
					$status.removeClass('status-ok status-error');
					$status.addClass('status-warning');
					break;
				case "Error":
					$status.removeClass('status-ok status-warning');
					$status.addClass('status-error');
					break;
				default:
					$status.removeClass('status-warning status-error');
				...
				if (data) {
					...
					$(auphonic_productions).each(function(key, value) {				
						var date = new Date(value.change_time),
							...
							title = value.output_basename + ' (' + datestring + ') [' + value.status_string + ']';
						$("#import_from_auphonic").append('<option value="' + value.uuid + '">' + title + '</option>\n');
					});
