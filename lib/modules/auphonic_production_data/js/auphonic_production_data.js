...
(function($){
	PODLOVE.Auphonic = function (container) {
		...
		var maybe_update_auphonic_metadata = function () {
			...
			if (!episode_slug || !episode_slug.length) return;
		...
		$("#_podlove_meta_slug").on('mediaFileHasUpdated', function(event, url) {
			if (url.match("json$")) maybe_update_auphonic_metadata();
		});
